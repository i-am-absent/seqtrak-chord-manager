...
Run: `MAIN_PID=$(systemctl show -p MainPID --value seqtrak-chord-manager.service); sudo kill "$MAIN_PID"; sleep 4; systemctl is-active seqtrak-chord-manager.service; systemctl show -p MainPID --value seqtrak-chord-manager.service`

Expected: `active` and a PID different from `$MAIN_PID`.
